...
def sort_by_range(x, y, points):
    ...
    if delta_x >= delta_y:
        for z in x_sorted:
            ...
            xy.append(list(points[points[:, 0] == z][0]))
    else:
        for z in y_sorted:
            # gdf_linhas.at[gdf_linhas.geometry.values[0].coords.xy[1] == z]
            xy.append(list(points[points[:, 1] == z][0]))

    return xy


def select_ref_coord_to_sort_rows(rows):

    x_middle = np.array([x.coords.xy[0][int(len(x.coords.xy[0]) / 2)] for x in rows.geometry.values])
    y_middle = np.array([x.coords.xy[1][int(len(x.coords.xy[1]) / 2)] for x in rows.geometry.values])
    points_middle = np.array([[x.coords.xy[0][int(len(x.coords.xy[0]) / 2)],
                               x.coords.xy[1][int(len(x.coords.xy[1]) / 2)]] for x in rows.geometry.values])

    xy_middle = sort_by_range(x_middle, y_middle, points_middle)



    x_first = np.array([x.coords.xy[0][0] for x in rows.geometry.values])
    y_first = np.array([x.coords.xy[1][0] for x in rows.geometry.values])
    points_first = np.array([[x.coords.xy[0][0],
                              x.coords.xy[1][0]] for x in rows.geometry.values])


    x_last = np.array([x.coords.xy[0][-1] for x in rows.geometry.values])
    y_last = np.array([x.coords.xy[1][-1] for x in rows.geometry.values])
    points_last = np.array([[x.coords.xy[0][-1],
                             x.coords.xy[1][-1]] for x in rows.geometry.values])


    xy_first = sort_by_range(x_first, y_first, points_first)
...
def sort_rows(rows):
    ...
    xy = sort_by_range(x, y, points_selected)
    ...
    rows['ref_coord'] = [str([round(x, 5), round(y, 5)]) for x, y in zip(x, y)]

    for i, coord in enumerate(xy):
        rows.at[rows['ref_coord'] == coord, 'id_sort'] = i + 1

    rows.drop('ref_coord', axis=1, inplace=True)

    return rows


def buffer_to_rows(buffer, points, pt_inverse=False, final_sort=False):
    rows = []
    points['buffer'] = -99
    points['id_row'] = -99
    ...
    for i in buffer.index:
        ...
        if final_sort:
            for id_row_new, geom in zip(id_points_in_row, xy):
                points.at[points['geometry']==Point(geom), 'id_row'] = id_row_new
        else:
...
def detect_gaps(points, rows):
    ...
    points_new = pd.concat([points, gaps], axis=0, ignore_index=True)
    points_new['id_i'] = points_new['id']
    points_new.set_index('id_i', inplace=True)
...
def detect_lines_uni(df_points, row_inverse=False, pt_inverse=False):
    ...
    _ = buffer_to_rows(buffer, points_new, pt_inverse=pt_inverse, final_sort=True)
    ...
    # Criar arquivo de rows so aqui
    ids_rows = []
    geom_rows = []
    for i in list(set(points_new.row.values)):
        points_new_clip = points_new.loc[points_new['row'] == i, :]

        ids_rows.append(i)
        geom_rows.append(LineString(points_new_clip.sort_values('id_row').geometry.values))
